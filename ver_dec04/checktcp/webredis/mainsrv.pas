...
procedure THTTPSRVForm.Timer2Timer(Sender: TObject);
    ...
    procedure Execute;
    var
     freq, cnt1, cnt2 :int64;
     delta: int64;
    begin
      inherited;
      QueryPerformanceFrequency(freq);
      QueryPerformanceCounter(cnt1);
      while not lclTerminated do begin
          application.ProcessMessages;
          QueryPerformanceCounter(cnt2);
          delta := (cnt2-cnt1)*1000000 div freq;//
          if delta<40000 then continue;

          TCExists := Compartido^.State;
          if TCExists then MyShift := Compartido^.Shift;


          if (MyShift <> MyShiftOld) then begin
            ini.WriteFloat('SYNH', 'Shift', MyShift);
            if MySinhro = chltc
              then ini.WriteInteger('SINH', 'Type', 0)
              else ini.WriteInteger('SINH', 'Type', 1);
            MyShift := MyShiftOld;
          end;
          ...
          if TCExists
            then TimeCode_Caption := MyDateTimeToStr(now - TimeCodeDelta)
            else TimeCode_Caption := '*' + MyDateTimeToStr(now - TimeCodeDelta);
