    ...
    begin
      ...
      begin
        TLZone.TLEditor.Events[i].SetPhraseText('ShortNum',
          txt + inttostr(i + smsh + 1));
        TLZone.TLEditor.Events[i].SetPhraseData('ShortNum', i + smsh + 1);
      end;
      for i := 0 to TLZone.Timelines[ps].Count - 1 do
      begin
        TLZone.Timelines[ps].Events[i].SetPhraseText('ShortNum',
          txt + inttostr(i + smsh + 1));
        TLZone.Timelines[ps].Events[i].SetPhraseData('ShortNum', i + smsh + 1);
      end;
    end;
  end;
end;
