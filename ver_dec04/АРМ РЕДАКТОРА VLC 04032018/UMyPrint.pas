...
function CountDeviceEvents(dev : integer) : integer;
...
var
  ...
  flag, flag1: word;
begin
  //if PrintListEvents.Count <= 0 then exit
  bmp := tbitmap.Create;
  try
  ...
  try
    ...
    for i := posi to PrintListEvents.Count - 1 do begin
      case tp of
        1: begin
             cntdev := 1;
             CurH := DrawMaketTableRow(bmp.Canvas, 0, i, CurH, MaketOptions.Bottom);
           end;
        2 .. 34: begin
            dvs := tp - 1;
            cntdev := CountDeviceEvents(dvs);
            if cntdev > 0 then begin
              if PrintListEvents.Rows[i].Device = dvs then
                CurH := DrawMaketTableRow(bmp.Canvas, 0, i, CurH,
                  MaketOptions.Bottom);
            end;
      ...
      if CurH > MaketOptions.LowBorder - 1 then begin
        ...
        if i < PrintListEvents.Count - 1 then begin
          if cntdev > 0 then  CountPages := CountPages + 1;
...
begin
  ...
  for i := 0 to PrintListEvents.Count - 1 do begin
     if PrintListEvents.Rows[i].Device = dev then result := result + 1;
  end;
end;
...
begin
  for j := 1 to SpinEdit1.Value do begin
    ...
    if tp = 0 then begin
      ...
      for i := 0 to cnt - 1 do begin
        if CountDeviceEvents(i+1) > 0 then begin
