...
function ReceiveBlob(sock: thandle; var buff: array of ansichar; len: integer;long:boolean): Integer;
var
    rc: integer;
    incount: integer;
    readLen: integer;
...
begin
    ...
    repeat
        if long then sleep(100) else sleep(2);
...
//        synWriteLog('webget', ' TCP To read = ' + IntToStr(incount));
        ...
        readLen := readLen + rc;
        if long and (incount<=0) then sleep(100);
        rc := ioctlsocket(sock, FIONREAD, incount);
        if ( long and (readlen<65000) ) and (timegettime<1000) then incount:=1;
    until incount <= 0;
...
//            webWriteLog('webget', ' not connectet delay after error');
    ...
    if long then begin
        webWriteLog('aa',' Sleep before long');
          sleep(300);
      end;

    rc := ReceiveBlob(tcpCli.Handle, INBUF, 200000,long);
    if rc < 0 then begin
        rc := WSAGetlastError;
    ...
    resp := INBUF;
    webWriteLog('webget', '!!! success. TIME = ' + IntToStr(timeGetTime - st) + '  len=' + IntToStr(length(resp)));
    webWriteLog('webget', '=====' + system.copy(resp, 1, 1111183));
    result := '';
