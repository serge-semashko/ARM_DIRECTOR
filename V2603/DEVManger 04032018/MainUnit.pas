...
begin
    ...
    WriteIniFile(AppPath + AppName + inttostr(ManagerNumber) + '.ini');
...
begin
    ...
    //str1 := ParamStr(0);
    //str2 := ParamStr(1);
    //if (ParamCount>=1) and (MyStrToInt(ParamStr(1))<>-1) then begin
...
//        str2:=stringreplace(str2,');','',[rfReplaceAll, rfIgnoreCase]);
    ...
    if (not startmanager) then begin
      ...
      (frm as TFrSetID).NotServer := not isserver;
      if ManagerNumber=-1
