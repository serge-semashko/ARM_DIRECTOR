...
uses
  ...
  PasLibVlcUnit, vlcpl, ustartwindow, ufrhotkeys, System.JSON, StrUtils;
...
Var
  ...
  ProgrammName: string = 'АРМ Режиссера';
  ProgrammVersia: string = 'v.2.0';
  ProgrammFullName: string = 'Автоматизированное Рабочее Место Режиссера';
  MainWindowStayOnTop: boolean = false;
  MainWindowMove: boolean = true;
  MainWindowSize: boolean = true;
  MainWindowBorderStyle: boolean = false;
...
procedure MyTextRect(var cv: tcanvas; const rect: trect; Text: string; Flag : word);
...
begin
  ...
  if dur < TLParameters.Duration then dur := TLParameters.Duration;
  result := dur;
end;
...
begin
  psi := posex('.',stri,1);
  s0 := copy(stri,1,psi-1);
  pse := posex('.',stri,psi+1);
  s1 := copy(stri,psi+1,pse-psi-1);
  psi := posex('.',stri,pse+1);
  s2 := copy(stri,pse+1,psi-pse-1);
  s3 := copy(stri,psi+1,length(stri));
  result := set3chars(s0) + '.' + set3chars(s1)  + '.' + set3chars(s2) + '.' + set3chars(s3);
end;
...
begin
  ...
  s2 := copy(stri,pse+1,psi-pse-1);
  s3 := copy(stri,psi+1,length(stri));
  result := inttostr(strtoint(s0)) + '.' + inttostr(strtoint(s1)) + '.' +
...
function countcharlength(cv: tcanvas; text : ansistring) : real;
var cnt, k1, k2, k3, k4 : real;
    i, fs1, fs2, fs3 : integer;
begin
  fs1 := cv.TextWidth('0');
  result := 0;
  for i := 1 to length(text)
    do result := result + cv.TextWidth(text[i]) / fs1;
//  fs2 := trunc(cnt);
//  if fs2 = 0
//  then result := 1
//  else begin
...
procedure MyTextRect(var cv: tcanvas; const rect: trect; Text: string; Flag : word);
var
  wdth, fntsz, sz : integer;
  itl, undl, strk : boolean;
  rt : trect;
  tmp : tfastdib;
  len : real;
begin
  ...
  try
    ...
    tmp.SetSize(rect.Right - rect.Left, rect.Bottom - rect.Top, 32);
    tmp.clear(TColorToTfcolor(cv.Brush.Color));
    tmp.SetBrush(bs_solid, 0, colortorgb(cv.Brush.Color));
    rt.Left := 0;
    rt.Top := 0;
    rt.Right := tmp.Width;
    rt.Bottom := tmp.Height;
    len := countcharlength(cv, text);
    len := (tmp.Width / len) * 0.8;
    fntsz := trunc(len);
    if fntsz<=2
      then if len - fntsz > 0.5
              then fntsz := fntsz + 1;

    tmp.FillRect(rt);
    tmp.SetTransparent(true);
    if fsItalic in cv.Font.Style then itl := true else itl:=false;
    if fsBold in cv.Font.Style then sz := 3 else sz := 1;
    if fsUnderline in cv.Font.Style then undl := true else undl := false;
    if fsStrikeOut in cv.Font.Style then strk := true else strk := false;
    tmp.SetPen(ps_Solid, 2, colortorgb(Form1.Font.Color));
    tmp.SetTextColor(colortorgb(cv.Font.Color));
    tmp.SetFontEx(cv.Font.Name,fntsz,tmp.Height, sz, itl, undl, strk);
    tmp.DrawText(Text,rt,flag);
    tmp.SetTransparent(false);

    tmp.Draw(cv.Handle,Rect.Left, Rect.Top);
    cv.Refresh;
  finally
    tmp.Free;
    tmp := nil;
        ...
        2:
          begin
            if (GridTimeLines.Selection.Top < 1) or
              ...
              exit;
            //PutGridTimeLinesToServer(Form1.GridTimeLines,1); //ssssssjson
