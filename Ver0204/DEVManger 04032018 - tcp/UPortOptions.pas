...
begin
    ...
    updatetcpport := false;
    tmp := shortipadress(Get('IP Адрес:'));
    if trim(IPAdress)<>trim(tmp) then updatetcpport := true;
    IPAdress := tmp;
    tmp := Get('IP Порт');
    if trim(IPPort)<>trim(tmp) then updatetcpport := true;
    IPPort := tmp;
    tmp := Get('IP Абонент:');
    if trim(IPLogin)<>trim(tmp) then updatetcpport := true;
    IPLogin := tmp;
    tmp := Get('IP Пароль:');
    if trim(IPPassword)<>trim(tmp) then updatetcpport := true;
    IPPassword := tmp;
    //sipaddr := Get('IP Адрес WEB сервера:');
    //psi := posex(':'
