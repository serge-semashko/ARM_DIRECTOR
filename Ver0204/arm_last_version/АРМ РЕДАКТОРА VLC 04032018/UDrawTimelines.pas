...
Type
  ...
  TTLButton = (btplusH, btminusH, btplusW, btminusW, btprocent, btlock,
    btstatus, bttimeline, btreview, btnone, btdevman);
...
begin
  if (Y>Scaler.Rect.Top) and (Y<Scaler.Rect.Bottom) then Scaler.MoveMouse(cv, X, Y);
  // Application.ProcessMessages;
  if (Y>Edit.Rect.Top) and (Y<Edit.Rect.Bottom) then Edit.MoveMouse(cv, Grid, X, Y);
  // Application.ProcessMessages;
  if (Y>NamesTL.Rect.Top) and (Y<NamesTL.Rect.Bottom) then NamesTL.MoveMouse(cv, Grid, X, Y);
  // Application.ProcessMessages;
  if (Y>Review.Rect.Top) and (Y<Review.Rect.Bottom) then Review.MouseMove(cv, X, Y);
end;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    begin
      if result.Button=btdevman then begin
        if TLParameters.vlcmode <> play then begin
          SetProtocol((grid.Objects[0,result.Position+1] as ttimelineoptions), result.Position+1)
        end;
      end else begin
        for i := 0 to NamesTL.Count - 1 do
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if Result.Button=btreview then begin
      DevicesOn := Not DevicesOn;
      if DevicesOn then PutGridTimeLinesToServer(Form1.GridTimeLines);
    end;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    Result := Scaler.ClickScaler(cv, X, Y);
    if Result.Button <> btnone then
...
begin
  try
    ...
    imgRect.Top := Top + (height - 30) div 2;
    imgRect.Bottom := imgRect.Top + 30;
    //if Selection then
    //  cv.Brush.Color := Color
    //else
    ...
    if DevicesOn then begin
      cv.StretchDraw(imgrect,form1.imgeyeop.Picture.Graphic);
      cv.TextOut(imgrect.Right+10, imgrect.Top+5, 'Управл. вкл.');
    end else begin
      cv.StretchDraw(imgrect,form1.imgeyecl.Picture.Graphic);
      cv.TextOut(imgrect.Right+10, imgrect.Top+5, 'Управл. выкл.');
    end;
...
begin
  try
    ...
    for i := 0 to Count - 1 do
    begin
      if (X > Names[i].devmRect.Left + 2) and (X < Names[i].devmRect.Right - 2) and
        (Y + 2 > Names[i].devmRect.Top) and (Y - 2 < Names[i].devmRect.Bottom) then
      begin
        Result.ID := Names[i].IDTimeline;
        Result.Position := i;
        Result.Button := btdevman;
        exit;
      end;
...
begin
  try
    ...
    clrtmp := cv.Brush.Color;

    cv.FillRect(Rect);
    cv.Brush.Style := bsClear;

    wdt := cv.TextWidth('00')+2;
    hgh := cv.TextHeight('0');
    ...
    begin
      ...
      begin
        ...
        nm := Name;
        wtxt := cv.TextWidth(nm);
        hgh := (Rect.Bottom - Rect.Top - cv.TextHeight(nm)) div 2;
        if wtxt <= Rect.Right - imgRect.Right - wdt - 62
        then begin
          cv.TextOut(imgRect.Right + 10, Rect.Top + hgh, nm);
        end else begin
          cv.Brush.Style := bsSolid;
          cv.Brush.Color := clrtmp;
          rt.Left := imgRect.Right + 10;
          rt.Top := Rect.Top + hgh;
          rt.Right := Rect.Right - wdt - 60;
          rt.Bottom := Rect.Bottom - hgh;
          MyTextRect(cv, rt, nm, DT_Left);
          cv.Brush.Style := bsClear;
        end;
      end;
      ...
      //if Text<>'Media' then begin
        s := (Grid.Objects[0, APos + 1] as TTimelineOptions).Manager;
        wdt := cv.TextWidth('00')+2;
        hgh := cv.TextHeight('0');
        devmRect.Right := BlockRect.Left - 8;
        devmRect.Left := devmRect.Right - wdt - 2;
        devmRect.Top := Rect.Top + (Rect.Bottom-Rect.Top-hgh) div 2 - 1;
        devmRect.Bottom := devmRect.Top + hgh + 2;
        cv.Brush.Color := clrtmp;
        cv.Rectangle(devmRect);
        s := (Grid.Objects[0, APos + 1] as TTimelineOptions).Manager;
        cv.Brush.Style := bsClear;
        cv.TextOut(devmRect.Left + (wdt + 2 - cv.TextWidth(s)) div 2, devmRect.Top + 1, s);
        //cv.TextOut(devmRect.Left + (wdt + 2 - cv.TextWidth('00')) div 2, devmRect.Top + 1, '00');
      //end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        wtxt := cv.TextWidth(nm);
        //hgh := (Rect.Bottom - Rect.Top - cv.TextHeight(nm)) div 2;
        if wtxt <= Rect.Right - imgRect.Right - 22
        then begin
          cv.TextOut(imgRect.Right + 10, Rect.Top + OffsetTextY, nm);
          //cv.TextOut(imgRect.Right + 10, Rect.Top + hgh, nm);
        end else begin
          cv.Brush.Style := bsSolid;
          cv.Brush.Color := Color;
          rt.Left := imgRect.Right + 10;
          rt.Top := Rect.Top + OffsetTextY;
