...
//    FrMyOptions.ComboBox1.Height);
  ...
  ps := ps + FrMyOptions.ComboBox1.Height;
  Groups[ARow].adddata('IP Адрес Web-сервера',
    server_addr, tomIPAdress, 10, ps, 300,
    FrMyOptions.ComboBox1.Height);

  ps := ps + FrMyOptions.ComboBox1.Height;
  Groups[ARow].adddata('IP Порт Web-сервера',
...
begin
  ...
  if trim(Groups[APos].getdata('Включить логирование')) = 'Да' then begin
    MakeLogging := true;
    FullLogMode := true;
  end else begin
    MakeLogging := false;
...
//    jsonware_url := s;
  ...
  s := shortipadress(trim(Groups[APos].getdata('IP Адрес Web-сервера')));
  if s = '' then
  ...
  s := trim(Groups[APos].getdata('IP Порт Web-сервера'));
  if s = '' then
