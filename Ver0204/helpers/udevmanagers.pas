...
var
    ...
    DevManagers_lastupdate: double = -1;
    DevManagers_UpdatePeriod: int64 = 500; //милисекунд
...
var
    ...
    str1, str2: string;
begin
    if (Now - DevManagers_lastupdate) * 24 * 3600 * 1000 < DevManagers_UpdatePeriod then begin
        sleep(0);
    end;
    DevManagers_lastupdate := now;
    for i := low(DevManagers) to high(DevManagers) do begin
        str2 := GetJsonStrFromServer('DEVMAN[' + inttostr(i) + ']');
        if DevManagers_old[i] <> str2 then begin
