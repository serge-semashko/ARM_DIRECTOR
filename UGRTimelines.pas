            ...
            if TLZone.TLEditor.Count > 0 then begin
              ...
              if TLParameters.Finish > TLZone.TLEditor.Events
                ...
                TLZone.TLEditor.Events[TLZone.TLEditor.Count - 1].Finish :=
                  TLParameters.Finish;
              if TLParameters.Finish < TLZone.TLEditor.Events
                [TLZone.TLEditor.Count - 1].Finish then begin
                ...
                if TLZone.Timelines[i].TypeTL=tldevice then begin
                  if TLZone.Timelines[i].Count>0 then begin
                    ...
                    if TLParameters.Finish < TLZone.Timelines[i].Events
                      ...
                      if TLParameters.Finish >
                        (TLZone.Timelines[i].Events[TLZone.Timelines[i].Count - 1].Start +
                        TLZone.Timelines[i].Events[TLZone.Timelines[i].Count - 1].SafeZone)
                      then
                        TLZone.Timelines[i].Events[TLZone.Timelines[i].Count - 1].Finish :=
                          TLParameters.Finish
                      else
...
function TTLTimeline.FindEventPos(evframe: longint): integer;
var
  i: integer;
begin
  result := -1;
...
begin
  try
    ...
    begin
      ...
      MyTextRect(cv, rts, txt, DT_Center);
...
begin
  try
    ...
    mpn := cv.Pen.Mode;
    cfn := cv.Font.Color;
    //lenev := TLParameters.EndPoint;
    if (TLParameters.EndPoint = 0) or (TLParameters.EndPoint = TLParameters.Preroll)
       then TLParameters.EndPoint := TLParameters.Duration;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          rts.Left := rt.Left;
          rts.Right := rt.Right;
          cv.Font.Color:=evnt.FontColor;//SmoothColor(clWhite,4);
          cv.Font.Size := evnt.FontSize;
          cv.Brush.Style := bsSolid;
          MyTextRect(cv, rts, txt, DT_Center);
        end
...
begin
  try
    ...
    try
      ...
      //dur := TLDuration;
      tmpdur := TLDuration;
      bmp.Width := (TLParameters.Preroll + tmpdur +
        TLParameters.Postroll) * TLParameters.FrameSize;
