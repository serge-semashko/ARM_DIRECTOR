...
uses umain, ucommon, ugrid, utimeline, umyfiles, uwebget, uwebredis_common, ugrtimelines;
...
begin
  try
    ...
    try
      ...
      dch := formatSETTINGS.DecimalSeparator;
      if dch = '.' then formatSETTINGS.DecimalSeparator := ',';
      ini.WriteFloat('SYNH', 'Shift', MyShift);
      formatSETTINGS.DecimalSeparator := dch;
      ...
      ini.WriteString('MAIN', 'server_addr', server_addr);
      ini.WriteString('MAIN', 'server_port', server_port);
      ...
      ini.WriteBool('MAIN', 'MainWindowBorderStyle', MainWindowBorderStyle);
      ini.WriteString('MAIN', 'ProgrammName', ProgrammName);
      ini.WriteString('MAIN', 'ProgrammVersia', ProgrammVersia);
      ini.WriteString('MAIN', 'ProgrammFullName', ProgrammFullName);
...
begin
  try
    ...
    try
      ...
      server_addr := ini.ReadString('MAIN', 'server_addr', server_addr);
      server_port := ini.ReadString('MAIN', 'server_port', server_port);
      ...
      MainWindowBorderStyle := ini.ReadBool('MAIN', 'MainWindowBorderStyle', MainWindowBorderStyle);
      ProgrammName := ini.ReadString('MAIN', 'ProgrammName', ProgrammName);
      TLParameters.ProgName := ProgrammName;
      ProgrammVersia := ini.ReadString('MAIN', 'ProgrammVersia', ProgrammVersia);
      TLParameters.ProgVersia := ProgrammVersia;
      ProgrammFullName := ini.ReadString('MAIN', 'ProgrammFullName', ProgrammFullName);

      MakeLogging := ini.ReadBool('MAIN', 'MakeLogging', MakeLogging);
      FullLogMode := MakeLogging;
