...
begin
  ...
  begin
    if trim(lowercase(Name)) = trim(lowercase(Lists[i].Name)) then
    begin
...
begin
  ...
  for i := 0 to count - 1 do
  begin
    if trim(lowercase(Lists[i].Name)) = trim(lowercase(Name)) then
    begin
      lst.Clear;
      for j := 0 to Lists[i].count - 1 do
        lst.Add(Lists[i].Lists[j].Value);
      result := i;
      exit;
    end;
  end;
...
begin
  ...
  for i := 0 to Lists[index].count - 1 do
    lst.Add(Lists[index].Lists[i].Value);
end;
...
ProgrammLists.Lists[AIndex].Lists[APos].Lists[AFld].TypeField :=
  'shorttimecode';
AFld := ProgrammLists.Lists[AIndex].Lists[APos].AddField('Set');
ProgrammLists.Lists[AIndex].Lists[APos].Lists[AFld].visible := false;
ProgrammLists.Lists[AIndex].Lists[APos].Lists[AFld].TypeObject := 'Phrase';
