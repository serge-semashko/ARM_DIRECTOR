...
begin
    ...
    Label2.Font.Size := ProgrammFontSize;
...
//    HideItem.Enabled := false;
    ...
    if str2<>'' then begin
      ...
      if str2<>'' then begin
        Label1.Caption:='DEVManager с номером ' + inttostr(ManagerNumber) +
                        ' уже запущен.' + #13#10 + 'Введите другой номер.';
        Panel3.Visible:=true;
        //repeat
        //  application.ProcessMessages;
        //until setnewnumber;
...
begin
  ...
  if trim(Edit1.Text)<>'' then begin
    ...
    if num>16 then label1.Caption:='Номер не может быть меньше 0 и больше 16.';
    str1 := 'DEVMAN[' + inttostr(num) +']';
    ...
    str2:=stringreplace(str2,');','',[rfReplaceAll, rfIgnoreCase]);
    if str2<>'' then begin
      Label1.Caption:='DEVManager с номером ' + inttostr(num) +
                      ' уже запущен.' + #13#10 + 'Введите другой номер.';
      exit;
    end;
  end;
        ...
        begin
          if trim(TLO_server[NumberTimeline].Protocol) <> '' then
            ...
            if sprot<>STRProtocol then begin
              ...
              SaveAProtocolToFile('AProtocol.txt', sprot);
              LoadProtocol('AListProtocols.txt', 'TLDevices', INFOTypeDevice, INFOVendor,
                           INFODevice, INFOProt);
              STRProtocol:=sprot;
