...
{$ENDIF}
  ...
  public
    Function getClientSocket: TCustomIpClient;
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Open; override;
    procedure Close; override;
...
//      FClientSocket := TCustomIpClient.Create(nil);
      try
        FServerSocketThread.ServerSocket.doAccept(FClientSocket);
      finally
        FClientSocket.close;
//        rc := closesocket(FClientSocket.fsocket);
//        if rc = SOCKET_ERROR then begin
//            rc := WSAGetlastError;
//            if rc <> 0 then begin
//                absWriteLog(IntToStr(rc) + ' Close HTTP client  ' + syserrormessage(rc) + ' ');
//            end;
//        end;
        FClientSocket.free;
        FClientSocket := nil;
...
begin
  ...
  begin
    if FServerSocket.WaitForConnection then
      if not Terminated then
      begin
        if (FThreadCacheSize = 0) then begin
            try
               self.serversocket.accept;
            Except
                  On E : Exception do;
                     //abswritelog('FThreadCacheSize = 0 accept error Error: '+E.Message);
            end;
        end else begin
            ...
            if Assigned(T) then begin
              t.clientsocket := serversocket.getClientSocket;
              T.Resume;
            end  else;//           absWriteLog('TCP server  ERROR!!! No socket');
    //        Sleep(0);
            end;
      end;
  end;
...
{$ENDIF}
  ...
  end else   ;//absWriteLog('HTTP SERVER Error on ACCEPT @@@@@@ ');
;
end;

function TCustomTcpServer.getClientSocket: TCustomIpClient;
var
  sock: TSocket;
  addr: TSockAddr;
  len: Integer;
