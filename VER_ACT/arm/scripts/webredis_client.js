...
//    setInterval(serial_net_process, 1);
    ...
    urlTail = "&callback=?";
    setInterval(async_net_process, 30);
}
...
//        console.log("\n!!! inflate error= " + e.name + " " + strLST.substr(761900, 762000) + "\"");
...
function getTLO() {
    ...
    try {
        ...
        if (checkObjectOk(t, lastTLOtime)) {
            ...
            lastTLOtime = +t.time;
//            console.log("lastTLOtime = " + lastTLOtime);
...
//    console.log('### proc LST = ' + JSON.stringify(t));
    if (t.reqInfo != undefined) {
        ...
        if (procSynch) {
            var deltaServerLocal = t.sent - (new Date().getTime()) ;
            if (deltaSysTime > deltaServerLocal) {
                deltaSysTime = deltaServerLocal;
            }
        }
        if (reqInfo.Tp1.length > 0) {
            var spr = (reqInfo.tStart + "-" + tStart) + ":";
            for (var i = 0; i < reqInfo.Tp1.length; i++) {
                spr += "Tp1: " + (reqInfo.Tp1[i] - reqInfo.Tp1[i]) + " Ts: " + (reqInfo.Ts[i] - reqInfo.Tp1[i]) + " Tp2: " + (reqInfo.Tp2[i] - reqInfo.Tp1[i]) + " " + (reqInfo.Tp2[i] - reqInfo.Ts[i]) + " " + " n ";
            }
//            console.log('deltaServerLocal=' + deltaServerLocal + " spr=" + spr);
...
function getLST() {
    ...
    if ("object" == typeof t) {
        myLog("#AJAX  LST success" + JSO7N.stringify(t));
    } else {
...
function serial_net_process() {
    ...
    try {
        ...
        if (LST_OK) {
            ...
            $("#serverStatus1").css("color", serverReady);
        } else {
            $("#serverStatus").css("color", "red");
            $("#serverStatus1").css("color", "red");
        }
        if (TLT_OK && TLO_OK && TLP_OK) {
            $("#armStatus").css("color", serverReady);
            $("#progtitle1").css("color", serverReady);
        } else {
            $("#armStatus").css("color", "red");
            $("#progtitle1").css("color", "red");
        }
        if (firstEnter) {
            firstEnter = false;
            hidePage();
...
function processRequest(t) {
    ...
    if (checkObjectOk(t, lastLSTtime)) {
        ...
        // myLog("delta Finish"+(new Date().getTime() - pFinish));
        pFinish = new Date().getTime();
        proc_LST(t);
        $("#serverStatus").css("color", serverReady);
        $("#serverStatus1").css("color", serverReady);
        if (TLT_OK && TLO_OK && TLP_OK) {
