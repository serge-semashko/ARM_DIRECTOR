...
//    webWriteLog('GETVAR>', 'Get ' + varName);
    ...
    for i := 0 to webvar_count - 1 do begin
        ...
        if webvars[i].changed = -1 then begin
            webWriteLog('GETVAR>>', 'Got ' + varName + ' ПУСТО');
            exit;
        end;
...
begin
    ...
    if json <> nil then begin
        ...
        for i_json := 0 to json.Count - 1 do begin
            ...
            if (AnsiLowerCase(varName) = 'ctc_shift') then begin
                ctc_shift :=-1;
                val(varValue, convValue, convResult);
                if convResult <> 0 then begin
                    ctc_shift :=-1;
            ...
            if (AnsiLowerCase(varName) = 'ctc_shift_frame') then begin
                ctc_shift_frame :=-1;
                val(varValue, convValue, convResult);
                if convResult <> 0 then begin
                    ctc_shift_frame :=-1;
...
begin
    ...
    if ans = '' then begin
        ...
        if (pos('"RC":0', resp) > 0) and (trim(webvars[ID].jsonStr) = '') then begin
            webvars[ID].changed := -1;
            webWriteLog(' TO SERVER>', ' Deleted '+ webvars[ID].Name+' changed = '+IntToStr(webvars[ID].changed));
        end;
    end;

    webWriteLog('TO SERVER>', 'Sent ' + webvars[ID].Name + ' res=' + resp);
end;
...
initialization
    ...
    fastvars.Add('TLP');
    fastvars.Add('CTC');
    for i:= 0 to 16 do  fastvars.Add('DEVMAN['+IntToStr(i)+']');
