...
begin
  try
    ...
    on E: Exception do
      WriteLog('MAIN', 'UMyFiles.ADDBitmapToList | ' + E.Message);
  end;
end;

function selectmaxbmp: TPoint;
var
  i, posmax, szw, szh: integer;
begin
  try
    result.X := 0;
    result.Y := 0;
    if listbitmap.Count <= 0 then
      exit;
    posmax := 0;
    szw := 0;
    szh := 0;
    for i := 0 to listbitmap.Count - 1 do
    begin
      pbmp := listbitmap[i];
      if (pbmp^.Width >= szw) then
      begin
        if (pbmp^.Height > szh) then
        begin
          szh := pbmp^.Height;
          posmax := i;
        end;
    ...
    WriteLog('MAIN', 'UMyFiles.selectmaxbmp Width=' + inttostr(result.X) +
      ' Height=' + inttostr(result.Y));
  except
    on E: Exception do
      WriteLog('MAIN', 'UMyFiles.selectmaxbmp Width=' + inttostr(result.X) +
        ' Height=' + inttostr(result.Y) + ' | ' + E.Message);
  end;
end;
...
begin
  try
    ...
    if tmpw > cellwdth then
    begin
      result.X := cellwdth;
      result.Y := trunc(bmphght / wdlt);
      WriteLog('MAIN', 'UMyFiles.BmpGetDivider-1 Width=' + inttostr(result.X) +
        ' Height=' + inttostr(result.Y));
      exit;
    end;

    if tmph > cellhght then
    begin
      result.X := trunc(bmpwdth / hdlt);
      result.Y := cellhght;
      WriteLog('MAIN', 'UMyFiles.BmpGetDivider-2 Width=' + inttostr(result.X) +
        ' Height=' + inttostr(result.Y));
      exit;
    end;
    ...
    result.X := tmpw;
    result.Y := tmph;
    WriteLog('MAIN', 'UMyFiles.BmpGetDivider-3 Width=' + inttostr(result.X) +
      ' Height=' + inttostr(result.Y));
  except
    on E: Exception do
      WriteLog('MAIN', 'UMyFiles.BmpGetDivider Width=' + inttostr(result.X) +
        ' Height=' + inttostr(result.Y) + ' | ' + E.Message);
  end;
  // wdlt := bmpwdth / cellwdth;
  // hdlt := bmphght / cellhght;
  ...
  try
    ...
    try
      ...
      try
        ...
        WriteLog('MAIN', 'UMyFiles.JPEGtoBMP Out FileName=' +
    ...
    on E: Exception do
      WriteLog('MAIN', 'UMyFiles.JPEGtoBMP FileName=' + FileName + ' | ' +
        E.Message);
  end;
end;
...
begin
  try
    ...
    try
      bmp1.Canvas.CopyRect(bmp1.Canvas.ClipRect, src1.Canvas,
        src1.Canvas.ClipRect);
      bmp2 := tbitmap.Create;
      bmp2.Width := dst.Width;
      bmp2.Height := dst.Height;
      try
        bmp2.Canvas.CopyRect(bmp2.Canvas.ClipRect, src2.Canvas,
          src2.Canvas.ClipRect);
        while R.Right < dst.Width do
        begin
          R.Left := R.Right + 1;
          R.Right := R.Left + steplpi;
          if (i mod 2 <> 0) then
            dst.Canvas.CopyRect(R, src1.Canvas, R)
          else
            dst.Canvas.CopyRect(R, src2.Canvas, R);
          i := i + 1;
        end;
      finally
        bmp2.Free;
    ...
    on E: Exception do
      WriteLog('MAIN', 'UMyFiles.DrawLPIBitmap | ' + E.Message);
  end;
end;
