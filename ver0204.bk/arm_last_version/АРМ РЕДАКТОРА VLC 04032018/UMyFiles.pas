...
uses
  ...
  umytexttable, ufrprotocols, umyprotocols;
...
var
  ...
  ps: int64;
  cntw: Word;
  stlprotocol, sflprotocol, tempprotocol : ansistring;
  stlmanager, stypetl, tempmanager : string;
  indxt, indxv, indxd, indxp: Integer;
  strt,strv,strd,strp,psel : ansistring;
begin
  try
    try
      ...
      frSaveProject.ProgressBar1.Position := 65;
      hghtgr := 0;
      for j := 0 to Form1.GridTimeLines.RowCount - 1 do begin
        hghtgr := hghtgr + Form1.GridTimeLines.RowHeights[j];
        if Form1.GridTimeLines.Objects[0,j] is TTimelineOptions
        then begin
          stlmanager := (Form1.GridTimeLines.Objects[0,j]
                                as TTimelineOptions).Manager;
          stlprotocol := (Form1.GridTimeLines.Objects[0,j]
                                as TTimelineOptions).Protocol;
          if trim(stlprotocol)<>'' then begin

            if ListTypeDevices = nil then ListTypeDevices := TListTypeDevices.Create;
            ListTypeDevices.clear;
              case (Form1.GridTimeLines.Objects[0,j]
                    as TTimelineOptions).TypeTL of
            ...
            strt := GetProtocolsParam(stlprotocol, 'TypeDevices');
            strv := GetProtocolsParam(stlprotocol, 'Firms');
            strd := GetProtocolsParam(stlprotocol, 'Device');
            strp := GetProtocolsParam(stlprotocol, 'Protocol');
            psel := GetProtocolsParam(stlprotocol, 'PortSelect');
            ...
            if indxt <> -1 then begin
              ...
              if indxv <> -1 then begin
                ...
                indxd := ListTypeDevices.TypeDevices[indxt].Vendors[indxv]
                         .IndexOf(strd);
                if indxd = -1 then begin
                  if ListTypeDevices.TypeDevices[indxt].Vendors[indxv].Count > 0
                    then indxd := 0;
                end;
                if indxd <> -1 then begin
                  indxp := ListTypeDevices.TypeDevices[indxt].Vendors[indxv]
                          .FirmDevices[indxd].IndexOf(strp);
                  if indxp = -1 then begin
                    if ListTypeDevices.TypeDevices[indxt].Vendors[indxv]
                          .FirmDevices[indxd].Count>0 then indxp := 0;
                  end;
                  if indxp <> -1 then begin
//------------------------------------------------------------------------------
                    with ListTypeDevices do begin
                      tempprotocol := '<TypeDevices=' + TypeDevices[indxt].TypeDevice + '>';
                      with TypeDevices[indxt] do
                      begin
                        tempprotocol := tempprotocol + '<Firms=' + Vendors[indxv].Vendor + '>';
                        with Vendors[indxv] do
                        begin
                          tempprotocol := tempprotocol + '<Device=' + FirmDevices[indxd].Device + '>';
                          with FirmDevices[indxd] do
                          begin
                            tempprotocol := tempprotocol + '<Protocol=' + ListProtocols[indxp].Protocol + '>';
                            if trim(psel)<>'' then begin
                              if trim(psel) = 'RS422' then begin
                                if ListProtocols[indxp].Ports.exist422
                                  then tempprotocol := tempprotocol
                                                      + '<PortSelect=RS422>'
                                  else tempprotocol := tempprotocol
                                                     + '<PortSelect=IPAdress>';
                              end else begin
                                if ListProtocols[indxp].Ports.existip
                                  then tempprotocol := tempprotocol
                                                      + '<PortSelect=IPAdress>'
                                  else tempprotocol := tempprotocol
                                                     + '<PortSelect=RS422>';
                              ...
                              if ListProtocols[indxp].Ports.select422
                                then tempprotocol := tempprotocol
                                                     + '<PortSelect=RS422>'
                                else tempprotocol := tempprotocol
                                                     + '<PortSelect=IPAdress>';
                            end;
                            tempprotocol := tempprotocol + ListProtocols[indxp].Ports.GetString;
                            tempprotocol := tempprotocol + ListProtocols[indxp].ProtocolMain.GetString;
                            tempprotocol := tempprotocol + ListProtocols[indxp].ProtocolAdd.GetString;
                          end;
                        end;
                      end;
                    end;
//------------------------------------------------------------------------------
                    if trim(tempprotocol) = '' then begin
                      (Form1.GridTimeLines.Objects[0,j] as TTimelineOptions)
                        .Manager := '';
                      (Form1.GridTimeLines.Objects[0,j] as TTimelineOptions)
                        .Protocol := '';
                    end else begin
